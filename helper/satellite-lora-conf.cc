...
SatLoraConf::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::SatLoraConf")
    ...
    .AddAttribute ("Standard", "Standard used for phy layer configuration",
                   EnumValue (SatLoraConf::SATELLITE),
                   MakeEnumAccessor (&SatLoraConf::m_phyLayerStandard),
                   MakeEnumChecker (SatLoraConf::SATELLITE, "Satellite",
                                    SatLoraConf::EU863_870, "EU863-870"))
  ;
  return tid;
}
...
SatLoraConf::SetEu863_870Conf (Ptr<LorawanMacGateway> gatewayMac)
{
  ...
  gatewayMac->SetBandwidthForDataRate (std::vector<double>{125000, 125000, 125000, 125000, 125000, 125000, 250000});
  gatewayMac->SetMaxAppPayloadForDataRate (std::vector<uint32_t>{59, 59, 59, 123, 230, 230, 230, 230});
}
...
SatLoraConf::SetEu863_870Conf (Ptr<LorawanMacEndDeviceClassA> endDeviceMac)
{
  ...
  endDeviceMac->SetBandwidthForDataRate (std::vector<double>{125000, 125000, 125000, 125000, 125000, 125000, 250000});
  endDeviceMac->SetMaxAppPayloadForDataRate (std::vector<uint32_t>{59, 59, 59, 123, 230, 230, 230, 230});
...
SatLoraConf::SetSatelliteConf (Ptr<LorawanMacGateway> gatewayMac)
{
  ...
  //LoraLogicalChannel::LoraLogicalChannel (double frequency, uint8_t minDataRate, uint8_t maxDataRate)
