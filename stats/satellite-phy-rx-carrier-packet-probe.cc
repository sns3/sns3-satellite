...
{
    static TypeId tid =
        TypeId("ns3::SatPhyRxCarrierPacketProbe")
            .SetParent<Probe>()
...
void
SatPhyRxCarrierPacketProbe::SetValue(uint32_t nPackets, const Address& address, bool statusFlag)
{
...
void
SatPhyRxCarrierPacketProbe::TraceSink(uint32_t nPackets, const Address& address, bool statusFlag)
{
    NS_LOG_FUNCTION(this << nPackets << address << statusFlag);
    if (IsEnabled())
    {
