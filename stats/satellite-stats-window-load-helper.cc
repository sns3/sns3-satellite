...
{
  static TypeId tid = TypeId ("ns3::SatStatsWindowLoadHelper")
    ...
    case SatStatsHelper::OUTPUT_SCALAR_FILE:
      {
        ...
        m_aggregator = CreateAggregator ("ns3::MultiFileAggregator",
                                         ...
                                         "GeneralHeading", StringValue (GetIdentifierHeading ("window_load")));
    ...
    case SatStatsHelper::OUTPUT_SCATTER_FILE:
      {
        ...
        m_aggregator = CreateAggregator ("ns3::MultiFileAggregator",
                                         "OutputFileName", StringValue (GetOutputFileName ()),
                                         "GeneralHeading", StringValue (GetTimeHeading ("window_load")));
        Ptr<MultiFileAggregator> aggregator = m_aggregator->GetObject<MultiFileAggregator> ();
    ...
    case SatStatsHelper::OUTPUT_CDF_FILE:
      {
        ...
        m_aggregator = CreateAggregator ("ns3::MultiFileAggregator",
                                         "OutputFileName", StringValue (GetOutputFileName ()),
                                         "GeneralHeading", StringValue (GetDistributionHeading ("window_load")));
        Ptr<MultiFileAggregator> aggregator = m_aggregator->GetObject<MultiFileAggregator> ();
    ...
    case SatStatsHelper::OUTPUT_CDF_PLOT:
      {
        ...
        NS_ASSERT (plotAggregator != nullptr);
        //plot->SetTitle ("");
        plotAggregator->SetLegend ("Window normalized load (in bps/Hz)",
                                   "Frequency");
        plotAggregator->Add2dDataset ("0", "0");
        plotAggregator->Set2dDatasetDefaultStyle (Gnuplot2dDataset::LINES);
...
{
  static TypeId tid = TypeId ("ns3::SatStatsRtnFeederWindowLoadHelper")
    .SetParent<SatStatsWindowLoadHelper> ()
  ;
  return tid;
}
      ...
      for (NetDeviceContainer::Iterator itDev = devs.Begin ();
           itDev != devs.End (); ++itDev)
        {
          ...
          for (ObjectVectorValue::Iterator itCarrier = carriers.Begin ();
            ...
            {
              if (DynamicCast<SatPhyRxCarrier> (itCarrier->second)->GetCarrierType () != SatPhyRxCarrier::CarrierType::RA_ESSA)
                {
                	continue;
                }
