...
bool
SatStatsLinkJitterHelper::DisconnectProbeFromCollector(Ptr<Probe> probe, uint32_t identifier)
{
    NS_LOG_FUNCTION(this << probe << probe->GetName() << identifier);
...
SatStatsFwdFeederDevLinkJitterHelper::DoInstallProbes()
{
    ...
    for (NodeContainer::Iterator it = sats.Begin(); it != sats.End(); ++it)
    {
        ...
        if (satOrbiterDev->TraceConnectWithoutContext("RxFeederLinkJitter", callback))
        {
...
void
SatStatsFwdUserDevLinkJitterHelper::UpdateIdentifierOnProbes()
{
    NS_LOG_FUNCTION(this);
...
void
SatStatsFwdUserMacLinkJitterHelper::UpdateIdentifierOnProbes()
{
    NS_LOG_FUNCTION(this);
...
void
SatStatsFwdUserPhyLinkJitterHelper::UpdateIdentifierOnProbes()
{
    NS_LOG_FUNCTION(this);
...
SatStatsRtnUserDevLinkJitterHelper::DoInstallProbes()
{
    ...
    for (NodeContainer::Iterator it = sats.Begin(); it != sats.End(); ++it)
    {
        ...
        // Connect the object to the probe.
        if (satOrbiterDev->TraceConnectWithoutContext("RxUserLinkJitter", callback))
        {
            NS_LOG_INFO(this << " successfully connected with node ID " << (*it)->GetId()
                             << " device #" << satOrbiterDev->GetIfIndex());
        }
