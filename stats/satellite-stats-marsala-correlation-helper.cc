...
{
    static TypeId tid = TypeId("ns3::SatStatsMarsalaCorrelationHelper").SetParent<SatStatsHelper>();
    return tid;
}
...
SatStatsMarsalaCorrelationHelper::DoInstall()
{
    ...
    case SatStatsHelper::OUTPUT_SCALAR_FILE: {
        ...
        m_aggregator = CreateAggregator("ns3::MultiFileAggregator",
                                        ...
                                        BooleanValue(true),
                                        "GeneralHeading",
                                        StringValue(GetIdentifierHeading("correlations")));
    ...
    case SatStatsHelper::OUTPUT_SCATTER_FILE: {
        ...
        m_aggregator = CreateAggregator("ns3::MultiFileAggregator",
                                        ...
                                        StringValue(GetOutputFileName()),
                                        "GeneralHeading",
                                        StringValue(GetTimeHeading("correlations")));
    ...
    for (NodeContainer::Iterator it = gws.Begin(); it != gws.End(); ++it)
    {
        ...
        for (NetDeviceContainer::Iterator itDev = devs.Begin(); itDev != devs.End(); ++itDev)
        {
            ...
            for (ObjectVectorValue::Iterator itCarrier = carriers.Begin();
                 ...
                 ++itCarrier)
            {
                Ptr<SatPhyRxCarrierMarsala> c =
                    DynamicCast<SatPhyRxCarrierMarsala>(itCarrier->second);
                if (!c)
                {
                    continue;
                }
