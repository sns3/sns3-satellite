...
TypeId
SatLooModel::GetTypeId(void)
{
    static TypeId tid =
        TypeId("ns3::SatLooModel").SetParent<SatBaseFader>().AddConstructor<SatLooModel>();
    return tid;
}
...
SatLooModel::SatLooModel(Ptr<SatLooConf> looConf,
...
SatLooModel::ConstructDirectSignalOscillators()
{
    ...
    for (uint32_t i = 0; i < m_numOfStates; i++)
    {
        ...
        for (uint32_t j = 0; j < m_looParameters[i][3]; j++)
        {
            uint32_t n = j + 1;
            /// 1. Rotation speed
            /// 1a. Initiate \f[ \alpha_n = \frac{2\pi n - \pi + \theta}{4M},  n=1, 2, \ldots, M\f],
            ...
            /// value.
            double amplitude =
                m_normalRandomVariable->GetValue(m_looParameters[i][0], m_looParameters[i][1]);
...
SatLooModel::ConstructMultipathOscillators()
{
    ...
    for (uint32_t i = 0; i < m_numOfStates; i++)
    {
        ...
        for (uint32_t j = 0; j < m_looParameters[i][4]; j++)
        {
            ...
            std::complex<double> amplitude = std::complex<double>(std::cos(psi), std::sin(psi)) *
                                             2.0 / std::sqrt(m_looParameters[i][4]);
            /// 3. Construct oscillator:
            oscillators.push_back(CreateObject<SatFadingOscillator>(amplitude, phi, omega));
        }
...
SatLooModel::GetChannelGain()
{
    ...
    /// Direct signal
    std::complex<double> directComplexGain =
        GetOscillatorCosineWaveSum(m_directSignalOscillators[m_currentState], timeInSeconds);

    /// Multipath
    std::complex<double> multipathComplexGain =
        GetOscillatorComplexSum(m_multipathOscillators[m_currentState], timeInSeconds);
