...
SatUtMacState::SwitchToReadyForLogon()
{
    ...
    if (m_rcstState == SatUtMacState::OFF_STANDBY || m_rcstState == SatUtMacState::NCR_RECOVERY ||
...
SatUtMacState::SwitchToReadyForTdmaSync()
{
    ...
    if (m_rcstState == SatUtMacState::READY_FOR_LOGON || m_rcstState == SatUtMacState::NCR_RECOVERY)
    {
...
SatUtMacState::CheckNcrRecoveryTimeout()
{
    ...
    if ((m_checkNcrRecoveryScheduled + m_ncrRecoveryTimeout <= Simulator::Now()) &&
        (GetState() == NCR_RECOVERY))
    {
        m_logOffCallback();
    }
}
