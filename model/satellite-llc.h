...
/**
 * \ingroup satellite
 * \brief SatLlc base class holds the UT specific SatBaseEncapsulator instances, which are
 * responsible of fragmentation, defragmentation, encapsulation and decapsulation. Encapsulator
 * class is thus capable of working in both transmission and reception side of the system. The
 * SatLlc base class holds base pointers of the encapsulators, but the actual encapsulator types
 * depend on the simulation direction:
 *
 * At GW:
 ...
 * control packets without encapsulation, decapsulation, fragmentation and packing is implemented to
 * the SatBaseEncapsulator class.
 *
 *  SatLlc is a pure virtual class, thus it is not supposed to be instantiated. A proper version of
 * the SatLlc is inherited: SatUtLlc at the UT and SatGwLlc at the GW. There is no LLC layer at the
 * satellite.
 */
class SatLlc : public Object
{
  public:
     ...
     */
    void AddEncap(Mac48Address source,
                  Mac48Address dest,
                  uint8_t flowId,
     ...
     */
    void AddDecap(Mac48Address source,
                  Mac48Address dest,
                  uint8_t flowId,
