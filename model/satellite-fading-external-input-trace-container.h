...
class SatFadingExternalInputTraceContainer : public Object
{
  public:
     ...
     */
    typedef std::pair<Ptr<SatFadingExternalInputTrace>, Ptr<SatFadingExternalInputTrace>>
        ChannelTracePair_t;
     ...
     */
    Ptr<SatFadingExternalInputTrace> GetFadingTrace(uint32_t nodeId,
  ...
  private:
    ...
    /**
     * The name of file which defines the index table to be used for trace input of forward down
     * link for UT. Index table defines files used as trace input sources.
     */
    std::string m_utFwdDownIndexFileName;

    /**
     * The name of file which defines the index table to be used for trace input of return up link
     * for UTs. Index table defines files used as trace input sources.
     */
    std::string m_utRtnUpIndexFileName;

    /**
     * The name of file which defines the index table to be used for trace input of forward up link
     * for GWs. Index table defines files used as trace input sources.
     */
     ...
     */
    Ptr<SatFadingExternalInputTrace> CreateFadingTrace(
     ...
     *  Only external fading sources which are closer than (equal to) maximum allowed distance are
     * accepted. Maximum allowed distance is defined by attribute.
     *
     * \param container Container reference to find out needed trace file info
     * \param id Id of the node GW or UT (from SatIdMapper)
     * \param mobility Mobility for given node
     ...
     */
    std::string FindSourceBasedOnPosition(TraceFileContainer_t& container,
                                          uint32_t id,
                                          Ptr<MobilityModel> mobility);
};
