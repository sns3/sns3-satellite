...
SatCompositeSinrOutputTraceContainer::AddNode (key_t key)
{
  ...
  if (beamId < 0 || (utId < 0 && gwId < 0))
  ...
  else
    {
      if (utId >= 0 && gwId < 0)
        {
          filename << m_currentWorkingDirectory << "/src/satellite/data/compositesinrtraces/output/" << m_tag << "BEAM_" << beamId << "_UT_" << utId << "_channelType_" << SatEnums::GetChannelTypeName (key.second);
        }

      if (gwId >= 0 && utId < 0)
        {
          filename << m_currentWorkingDirectory << "/src/satellite/data/compositesinrtraces/output/" << m_tag << "BEAM_" << beamId << "_GW_" << gwId << "_channelType_" << SatEnums::GetChannelTypeName (key.second);
        }
