...
SatCompositeSinrOutputTraceContainer::AddNode (key_t key)
{
  ...
  if (traceId < 0)
    ...
    {
      filename << m_currentWorkingDirectory << "/src/satellite/data/compositesinrtraces/output/id_" << traceId << "_channelType_" << SatEnums::GetChannelTypeName (key.second) << m_tag;
