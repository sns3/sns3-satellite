...
SatCompositeSinrOutputTraceContainer::AddNode(key_t key)
{
    ...
    else
    {
        if (utId >= 0 && gwId < 0)
        {
            filename << dataPath << "/composite_sinr_output_trace_BEAM_" << beamId << "_UT_" << utId
                     << "_channelType_" << SatEnums::GetChannelTypeName(key.second);
        }

        if (gwId >= 0 && utId < 0)
        {
            filename << dataPath << "/composite_sinr_output_trace_BEAM_" << beamId << "_GW_" << gwId
                     << "_channelType_" << SatEnums::GetChannelTypeName(key.second);
        }
        ...
        std::pair<container_t::iterator, bool> result = m_container.insert(
            std::make_pair(key,
                           CreateObject<SatOutputFileStreamDoubleContainer>(
                               filename.str().c_str(),
                               std::ios::out,
                               SatBaseTraceContainer::CSINR_TRACE_DEFAULT_NUMBER_OF_COLUMNS)));
...
SatCompositeSinrOutputTraceContainer::AddToContainer(key_t key, std::vector<double> newItem)
{
    ...
    {
        NS_FATAL_ERROR(
            "SatCompositeSinrOutputTraceContainer::AddToContainer - Incorrect vector size");
    }
