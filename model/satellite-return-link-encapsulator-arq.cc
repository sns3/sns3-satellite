...
SatReturnLinkEncapsulatorArq::SatReturnLinkEncapsulatorArq(Mac48Address encapAddress,
                                                           Mac48Address decapAddress,
                                                           Mac48Address sourceE2EAddress,
                                                           Mac48Address destE2EAddress,
                                                           uint8_t flowId,
                                                           uint32_t additionalHeaderSize)
    : SatReturnLinkEncapsulator(encapAddress,
                                decapAddress,
                                sourceE2EAddress,
                                destE2EAddress,
                                flowId,
...
Ptr<Packet>
SatReturnLinkEncapsulatorArq::NotifyTxOpportunity(uint32_t bytes,
                                                  uint32_t& bytesLeft,
                                                  uint32_t& nextMinTxO)
{
    ...
    if (!m_retxBuffer.empty())
    {
        ...
        if (context->m_pdu->GetSize() <= bytes)
        {
            ...
            EventId t = Simulator::Schedule(m_retransmissionTimer,
                                            &SatReturnLinkEncapsulatorArq::ArqReTxTimerExpired,
                                            this,
                                            context->m_seqNo);
            context->m_waitingTimer = t;

            NS_LOG_INFO("UT: << " << m_encapAddress << " sent a retransmission packet of size: "
    ...
    else if (!m_txQueue->IsEmpty() && m_seqNo->SeqNoAvailable())
    {
        ...
        if (packet)
        {
            ...
            NS_LOG_INFO("UT: << " << m_encapAddress << " sent a packet of size: "
...
SatReturnLinkEncapsulatorArq::ArqReTxTimerExpired(uint8_t seqNo)
{
    ...
    NS_LOG_INFO("At UT: " << m_encapAddress
    ...
    if (it != m_txedBuffer.end())
    {
        ...
        {
            NS_LOG_INFO("For UT: " << m_encapAddress << " max retransmissions reached for "
                                   << (uint32_t)(seqNo));
...
SatReturnLinkEncapsulatorArq::ReceivePdu(Ptr<Packet> p)
{
    ...
    NS_LOG_INFO("UT: " << m_encapAddress << " received a packet with SeqNo: " << (uint32_t)(seqNo));
                ...
                if (it2 == m_reorderingBuffer.end())
                {
                    ...
                    EventId id =
                        Simulator::Schedule(m_rxWaitingTimer,
                                            &SatReturnLinkEncapsulatorArq::RxWaitingTimerExpired,
                                            this,
                                            i);
    ...
    {
        NS_LOG_INFO("UT: " << m_encapAddress << " received a packet with SeqNo: " << sn
...
SatReturnLinkEncapsulatorArq::RxWaitingTimerExpired(uint32_t seqNo)
{
    ...
    NS_LOG_INFO("For UT: " << m_encapAddress << " max waiting time reached for SeqNo: " << seqNo);
...
SatReturnLinkEncapsulatorArq::SendAck(uint8_t seqNo) const
{
    ...
    NS_LOG_INFO("GW: " << m_decapAddress << " send ACK to UT: " << m_encapAddress
