...
SatGenericStreamEncapsulatorArq::SatGenericStreamEncapsulatorArq(Mac48Address encapAddress,
                                                                 Mac48Address decapAddress,
                                                                 Mac48Address sourceE2EAddress,
                                                                 Mac48Address destE2EAddress,
                                                                 uint8_t flowId,
                                                                 uint32_t additionalHeaderSize)
    : SatGenericStreamEncapsulator(encapAddress,
                                   decapAddress,
                                   sourceE2EAddress,
                                   destE2EAddress,
                                   flowId,
    ...
    static TypeId tid =
        TypeId("ns3::SatGenericStreamEncapsulatorArq")
            ...
            .AddAttribute("RxWaitingTime",
                          "Time to wait for a packet at the reception (GW) before moving onwards "
                          "with the packet reception.",
...
Ptr<Packet>
SatGenericStreamEncapsulatorArq::NotifyTxOpportunity(uint32_t bytes,
                                                     uint32_t& bytesLeft,
                                                     uint32_t& nextMinTxO)
{
    ...
    if (!m_retxBuffer.empty())
    {
        ...
        if (context->m_pdu->GetSize() <= bytes)
        {
            ...
            std::map<uint8_t, Ptr<SatArqBufferContext>>::iterator it =
                m_txedBuffer.find(context->m_seqNo);
            if (it != m_txedBuffer.end())
            {
                NS_FATAL_ERROR("Trying to add retransmission packet to txedBuffer even though it "
            ...
            // Create the retransmission event and store it to the context. Event is cancelled if a
            // ACK is received. However, if the event triggers, we shall send the packet again, if
            // the packet still has retransmissions left.
            EventId t = Simulator::Schedule(m_retransmissionTimer,
                                            &SatGenericStreamEncapsulatorArq::ArqReTxTimerExpired,
                                            this,
                                            context->m_seqNo);
            context->m_waitingTimer = t;

            NS_LOG_INFO("GW: << " << m_encapAddress << " sent a retransmission packet of size: "
                                  << context->m_pdu->GetSize()
                                  << " with seqNo: " << (uint32_t)(context->m_seqNo)
                                  << " flowId: " << (uint32_t)(m_flowId));
        ...
        {
            NS_LOG_INFO("Retransmission PDU: " << context->m_pdu->GetUid()
    ...
    else if (!m_txQueue->IsEmpty() && m_seqNo->SeqNoAvailable())
    {
        ...
        if (packet)
        {
            ...
            // ACK is received. However, if the event triggers, we shall send the packet again, if
            // the packet still has retransmissions left.
            arqContext->m_waitingTimer =
                Simulator::Schedule(m_retransmissionTimer,
                                    &SatGenericStreamEncapsulatorArq::ArqReTxTimerExpired,
                                    this,
                                    seqNo);
            ...
            {
                NS_FATAL_ERROR("Created packet of size: " << packet->GetSize()
                                                          << " is larger than the tx opportunity: "
                                                          << bytes);
            }

            NS_LOG_INFO("GW: << " << m_encapAddress << " sent a packet of size: "
                                  << packet->GetSize() << " with seqNo: " << (uint32_t)(seqNo)
                                  << " flowId: " << (uint32_t)(m_flowId));
...
SatGenericStreamEncapsulatorArq::ArqReTxTimerExpired(uint8_t seqNo)
{
    ...
    NS_LOG_INFO("At GW: " << m_encapAddress
    ...
    if (it != m_txedBuffer.end())
    {
        ...
        {
            NS_LOG_INFO("For GW: " << m_encapAddress << " max retransmissions reached for "
                                   << (uint32_t)(seqNo));
    ...
    {
        NS_LOG_INFO("Element not found anymore in the m_txedBuffer, thus ACK has been received "
                    "already earlier");
    }
}
...
SatGenericStreamEncapsulatorArq::ReceivePdu(Ptr<Packet> p)
{
    ...
    NS_LOG_INFO("GW: " << m_encapAddress << " received a packet with SeqNo: " << (uint32_t)(seqNo));
    ...
    if (sn >= m_nextExpectedSeqNo)
    {
        ...
        NS_LOG_INFO("Received a packet with SeqNo: " << sn
                                                     << ", expecting: " << m_nextExpectedSeqNo);
        ...
        if (sn != m_nextExpectedSeqNo)
        {
            ...
            {
                std::map<uint32_t, Ptr<SatArqBufferContext>>::iterator it2 =
                    m_reorderingBuffer.find(i);
                ...
                if (it2 == m_reorderingBuffer.end())
                {
                    ...
                    EventId id =
                        Simulator::Schedule(m_rxWaitingTimer,
                                            &SatGenericStreamEncapsulatorArq::RxWaitingTimerExpired,
                                            this,
                                            i);
    ...
    {
        NS_LOG_INFO("GW: " << m_encapAddress << " received a packet with SeqNo: " << sn
...
SatGenericStreamEncapsulatorArq::ConvertSeqNo(uint8_t seqNo) const
{
    ...
    NS_LOG_INFO("Input: " << (uint32_t)(seqNo) << " rounds: " << rounds << " rawSeqNo: " << rawSeqNo
                          << " windowSize: " << m_arqWindowSize
                          << " next expected: " << m_nextExpectedSeqNo);
...
SatGenericStreamEncapsulatorArq::ReassembleAndReceive()
{
    ...
    NS_LOG_INFO("Process SeqNo: " << it->first << ", expected: " << m_nextExpectedSeqNo
     ...
     * and erase it.
     */
    while (it != m_reorderingBuffer.end() && it->first == m_nextExpectedSeqNo &&
           it->second->m_rxStatus == true)
    {
        NS_LOG_INFO("Process SeqNo: " << it->first << ", expected: " << m_nextExpectedSeqNo
                                      << ", status: " << it->second->m_rxStatus);
...
SatGenericStreamEncapsulatorArq::RxWaitingTimerExpired(uint32_t seqNo)
{
    ...
    NS_LOG_INFO("For GW: " << m_encapAddress << " max waiting time reached for SeqNo: " << seqNo);
...
SatGenericStreamEncapsulatorArq::SendAck(uint8_t seqNo) const
{
    ...
    NS_LOG_INFO("GW: " << m_decapAddress << " send ACK to GW: " << m_encapAddress
                       << " with flowId: " << (uint32_t)(m_flowId)
    ...
    if (!m_ctrlCallback.IsNull())
    {
        ...
        m_ctrlCallback(ack, m_encapAddress);
    }
    else
    {
