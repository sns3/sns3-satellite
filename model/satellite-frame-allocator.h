 ...
 * with proper parameters. When needed allocations are done, symbols are shared between UTs by
 * calling PreAllocateSymbols. Finally time slots for UT can be generated for UT according to
 * preallocation by calling method GenerateTimeSlots. GenerateTimeSlots method can be called as
 * several time, if same preallocation is wanted to use.
 *
...
class SatFrameAllocator : public SimpleRefCount<SatFrameAllocator>
{
  public:
     ...
     *
     * key is UT's address and value is UtAllocInfoItem_t is vector holding information of the
     * allocated bytes for the UT/RC.
     */
    ...
    inline Ptr<SatFrameAllocator> GetParent() const
    {
        return m_parent;
    }
