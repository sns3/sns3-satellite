...
SatInterferenceOutputTraceContainer::AddNode(key_t key)
{
    ...
    else
    {
        if (utId >= 0 && gwId < 0)
        {
            filename << dataPath << "/interference_output_trace_BEAM_" << beamId << "_UT_" << utId
                     << "_channelType_" << SatEnums::GetChannelTypeName(key.second);
        }

        if (gwId >= 0 && utId < 0)
        {
            filename << dataPath << "/interference_output_trace_BEAM_" << beamId << "_GW_" << gwId
                     << "_channelType_" << SatEnums::GetChannelTypeName(key.second);
        }
...
SatInterferenceOutputTraceContainer::AddToContainer(key_t key, std::vector<double> newItem)
{
    ...
    {
        NS_FATAL_ERROR(
            "SatInterferenceOutputTraceContainer::AddToContainer - Incorrect vector size");
    }
