...
class SatEnums
{
  public:
    ...
    {
        TRANSPARENT,
        REGENERATION_PHY,  // regenerate only on physical layer, to compute is the packet is lost or
                           // not
        REGENERATION_LINK, // regenerate on physical and MAC layers (only on return link). Satellite
                           // can perform packet TX scheduling and handle control messages
        REGENERATION_NETWORK // regenerate all stack. The GSE/RLE encapsulation is removed at
                             // reception and added again when transmitting to next hop
    } RegenerationMode_t;
    ...
    typedef enum
    {
        UNICAST, // Only one route for a pair source satellite / destination satellite, using
                 // shortest path (in hops)
        ECMP, // Get all routes possible with minimum hops. For each incoming packet, the route is
              // randomly selected among all available
    } IslArbiterType_t;
    ...
    static inline std::string GetRandomAccessOpportunityTypeName(
        RandomAccessTxOpportunityType_t opportunityType)
    {
    ...
    static inline std::string GetRbdcCapacityRequestAlgorithmName(
        RbdcCapacityRequestAlgorithm_t rbdcAlgorithm)
    {
    ...
    static inline std::string GetVbdcCapacityRequestAlgorithmName(
        VbdcCapacityRequestAlgorithm_t vbdcAlgorithm)
    {
