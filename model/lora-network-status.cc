  ...
  if (m_endDeviceStatuses.find (edAddress) == m_endDeviceStatuses.end ())
    {
      ...
      Ptr<LoraEndDeviceStatus> edStatus = CreateObject<LoraEndDeviceStatus>
        (edAddress, edMac->GetObject<LorawanMacEndDeviceClassA>());

      // Add it to the map
      m_endDeviceStatuses.insert (std::pair<LoraDeviceAddress, Ptr<LoraEndDeviceStatus> >
                                  (edAddress, edStatus));
...
void
LoraNetworkStatus::OnReceivedPacket (Ptr<const Packet> packet,
...
LoraNetworkStatus::GetEndDeviceStatus (Ptr<Packet const> packet)
{
