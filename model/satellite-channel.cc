...
SatChannel::RemoveRx(Ptr<SatPhyRx> phyRx)
{
    ...
    PhyRxContainer::iterator phyIter =
        std::find(m_phyRxContainer.begin(), m_phyRxContainer.end(), phyRx);
             ...
             ++rxPhyIterator)
        {
            ...
            if ((*rxPhyIterator)->GetBeamId() == txParams->m_beamId)
            {
                ...
                case SatEnums::RETURN_FEEDER_CH: {
                    // Go through the packets and check their destination address by peeking the MAC
                    // tag
                    SatSignalParameters::PacketsInBurst_t::const_iterator it =
                        txParams->m_packetsInBurst.begin();
                    for (; it != txParams->m_packetsInBurst.end(); ++it)
                    {
                        ...
                        if (dest == (*rxPhyIterator)->GetAddress() || dest.IsBroadcast() ||
                            dest.IsGroup())
                        {
                            ScheduleRx(txParams, *rxPhyIterator);
...
SatChannel::DoRxPowerOutputTrace(Ptr<SatSignalParameters> rxParams, Ptr<SatPhyRx> phyRx)
{
    ...
    double carrierBandwidthHz = m_carrierBandwidthConverter(m_channelType,
                                                            rxParams->m_carrierId,
    ...
    case SatEnums::RETURN_FEEDER_CH:
    case SatEnums::FORWARD_USER_CH: {
        Singleton<SatRxPowerOutputTraceContainer>::Get()->AddToContainer(
            std::make_pair(phyRx->GetDevice()->GetAddress(), m_channelType),
            tempVector);
        break;
    }
    case SatEnums::FORWARD_FEEDER_CH:
    case SatEnums::RETURN_USER_CH: {
        Singleton<SatRxPowerOutputTraceContainer>::Get()->AddToContainer(
            std::make_pair(GetSourceAddress(rxParams), m_channelType),
            tempVector);
        break;
    }
    default: {
...
SatChannel::DoRxPowerInputTrace(Ptr<SatSignalParameters> rxParams, Ptr<SatPhyRx> phyRx)
{
    ...
    case SatEnums::FORWARD_USER_CH: {
        ...
        rxParams->m_rxPower_W =
            carrierBandwidthHz *
            Singleton<SatRxPowerInputTraceContainer>::Get()->GetRxPowerDensity(
                std::make_pair(phyRx->GetDevice()->GetAddress(), m_channelType));
    ...
    case SatEnums::RETURN_USER_CH: {
        // Calculate the Rx power from Rx power density
        rxParams->m_rxPower_W =
            carrierBandwidthHz * Singleton<SatRxPowerInputTraceContainer>::Get()->GetRxPowerDensity(
                                     std::make_pair(GetSourceAddress(rxParams), m_channelType));
        break;
    }
    default: {
...
SatChannel::DoRxCnoInputTrace(Ptr<SatSignalParameters> rxParams, Ptr<SatPhyRx> phyRx)
{
    ...
    double rxNoisePowerW =
        SatConstVariables::BOLTZMANN_CONSTANT * rxTemperatureK * carrierBandwidthHz;
    ...
    case SatEnums::FORWARD_USER_CH: {
        ...
        cno = Singleton<SatRxCnoInputTraceContainer>::Get()->GetRxCno(
            std::make_pair(phyRx->GetDevice()->GetAddress(), m_channelType));
        if (cno == 0)
        {
            DoRxPowerCalculation(rxParams, phyRx);
            NS_LOG_INFO("Use calculation downlink \t" << cno << " " << carrierBandwidthHz << " "
                                                      << rxNoisePowerW << " "
                                                      << rxParams->m_rxPower_W);
            return;
        }
        rxParams->m_rxPower_W = rxNoisePowerW * cno / carrierBandwidthHz;
        NS_LOG_INFO("Channel downlink \t" << cno << " " << carrierBandwidthHz << " "
                                          << rxNoisePowerW << " "
                                          << rxNoisePowerW * cno / carrierBandwidthHz);
        break;
    }
    case SatEnums::FORWARD_FEEDER_CH:
    case SatEnums::RETURN_USER_CH: {
        ...
        {
            DoRxPowerCalculation(rxParams, phyRx);
            NS_LOG_INFO("Use calculation uplink   \t" << cno << " " << carrierBandwidthHz << " "
                                                      << rxNoisePowerW << " "
                                                      << rxParams->m_rxPower_W);
            return;
        }
        rxParams->m_rxPower_W = rxNoisePowerW * cno / carrierBandwidthHz;
        NS_LOG_INFO("Channel uplink   \t" << cno << " " << carrierBandwidthHz << " "
                                          << rxNoisePowerW << " "
                                          << rxNoisePowerW * cno / carrierBandwidthHz);
        break;
    }
    ...
    NS_LOG_INFO("Target C/N0: " << cno << ", carrier bw: " << carrierBandwidthHz
                                << ", rxPower: " << SatUtils::LinearToDb(rxParams->m_rxPower_W)
                                << ", carrierId: " << rxParams->m_carrierId << ", channelType: "
                                << SatEnums::GetChannelTypeName(m_channelType));
...
void
SatChannel::DoFadingOutputTrace(Ptr<SatSignalParameters> rxParams,
                                Ptr<SatPhyRx> phyRx,
                                double fadingValue)
{
    ...
    case SatEnums::RETURN_FEEDER_CH:
    case SatEnums::FORWARD_USER_CH: {
        Singleton<SatFadingOutputTraceContainer>::Get()->AddToContainer(
            std::make_pair(phyRx->GetDevice()->GetAddress(), m_channelType),
            tempVector);
        break;
    }
...
SatChannel::DoRxPowerCalculation(Ptr<SatSignalParameters> rxParams, Ptr<SatPhyRx> phyRx)
{
    ...
    double rxPower_W = (rxParams->m_txPower_W * txAntennaGain_W) /
                       m_freeSpaceLoss->GetFsl(txMobility, rxMobility, rxParams->m_carrierFreq_hz);
    rxParams->m_rxPower_W =
        rxPower_W * rxAntennaGain_W / phyRx->GetLosses() * markovFading / extFading;
}
