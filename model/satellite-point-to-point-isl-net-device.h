...
class PointToPointIslNetDevice : public NetDevice
{
  ...
  private:
    ...
    DataRate m_dataRate; //!< The data rate that the Net Device uses to simulate packet transmission
                         //!< timing
    Time m_tInterframeGap; //!< The interframe gap that the Net Device uses to throttle packet
                           //!< transmission
    Ptr<PointToPointIslChannel> m_channel; //!< The PointToPointIslChannel to which this
                                           //!< PointToPointIslNetDevice has been attached
    Ptr<DropTailQueue<Packet>>
        m_queue; //!< The Queue which this PointToPointIslNetDevice uses as a packet source.
