...
class SatRandomAccess : public Object
{
  public:
     ...
     * \brief Random access Tx opportunities. This struct contains the algorithm results from this
     * module
     */
    typedef struct
    {
     ...
     * \brief Constructor
     */
    SatRandomAccess(Ptr<SatRandomAccessConf> randomAccessConf,
     ...
     */
    SatRandomAccess::RandomAccessTxOpportunities_s DoRandomAccess(
        uint32_t allocationChannel,
  ...
  private:
    ...
    /**
     * \brief Function to convert a carrier allocation channel ID into its allocation channel
     * configuration ID \param allocationChannelId The carrier allocation channel ID \return The
     * carrier allocation channel configuration ID
     */
     ...
     */
    std::pair<std::set<uint32_t>, std::set<uint32_t>> CrdsaRandomizeTxOpportunities(
        uint32_t allocationChannel,
     ...
     */
    SatRandomAccess::RandomAccessTxOpportunities_s CrdsaPrepareToTransmit(
        uint32_t allocationChannel);
    ...
    /**
     * \brief Function for increasing the allocation channel specific count of consecutive used
     * blocks. If the maximum number of consecutive blocks is reached, this function triggers the
     * call of idle period. \param allocationChannel allocation channel
     */
    void CrdsaIncreaseConsecutiveBlocksUsed(uint32_t allocationChannel);

    /**
     * \brief Function for increasing the count of consecutive used blocks for all allocation
     * channels
     */
    ...
    /**
     * \brief Function for resetting the number of consecutive blocks to zero for all allocation
     * channels
     */
    void CrdsaResetConsecutiveBlocksUsedForAllAllocationChannels();

    /**
     * \brief Function for resetting the number of consecutive blocks to zero for a specific
     * allocation channel \param allocationChannel allocation channel
     */
