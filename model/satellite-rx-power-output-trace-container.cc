...
SatRxPowerOutputTraceContainer::AddNode (key_t key)
{
  ...
  filename << m_currentWorkingDirectory << "/src/satellite/data/rxpowertraces/output/id_" << SatHelper::m_satMacIdMacMapper->GetId (key.first) << "_channelType_" << SatEnums::GetChannelTypeName (key.second);
...
SatRxPowerOutputTraceContainer::WriteToFile ()
{
  ...
  for (iter = m_container.begin (); iter != m_container.end (); ++iter)
    {
      iter->second->EnableFigureOutput ("Rx power trace","Time (s)","Rx power (dBW)","set key top right",SatOutputFileStreamDoubleContainer::DECIBEL);
