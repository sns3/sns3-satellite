...
class SatPhyRxCarrierPerSlot : public SatPhyRxCarrier
{
  public:
     ...
     */
    SatPhyRxCarrierPerSlot(uint32_t carrierId,
    ...
    {
        return m_randomAccessEnabled ? CarrierType::RA_SLOTTED_ALOHA
                                     : CarrierType::DEDICATED_ACCESS;
    }
  ...
  private:
     ...
     * \return PHY error
     */
    bool ProcessSlottedAlohaCollisions(
        double cSinr,
        Ptr<SatSignalParameters> rxParams,
    ...
    SatPhyRxCarrierConf::RandomAccessCollisionModel
        m_randomAccessCollisionModel;       //< Random access collision model
    double m_randomAccessConstantErrorRate; //< Constant error rate if collision model is
                                            // RA_CONSTANT_COLLISION_PROBABILITY
    uint32_t
        m_randomAccessAverageNormalizedOfferedLoadMeasurementWindowSize; //< Random access average
    ...
    std::deque<double>
        m_randomAccessDynamicLoadControlNormalizedOfferedLoad; //< Container for calculated
                                                               // normalized offered loads

    bool m_disableErrorHighTransmissionTime; //< Disable fatal error when transmission time is
                                             // higher than propagation time, but computations are
                                             // less precise
};
