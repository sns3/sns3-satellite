    ...
    std::pair<container_t::iterator, bool> result = m_container.insert(
        std::make_pair(filename,
                       CreateObject<SatInputFileStreamTimeDoubleContainer>(
                           filename.c_str(),
                           std::ios::in,
                           SatBaseTraceContainer::POSITION_TRACE_DEFAULT_NUMBER_OF_COLUMNS)));
...
GeoCoordinate
SatPositionInputTraceContainer::GetPosition(const std::string& key,
                                            GeoCoordinate::ReferenceEllipsoid_t refEllipsoid)
{
    NS_LOG_FUNCTION(this);

    std::vector<double> row = FindNode(key)->InterpolateBetweenClosestTimeSamples();
    return GeoCoordinate(row.at(SatBaseTraceContainer::POSITION_TRACE_DEFAULT_LATITUDE_INDEX),
