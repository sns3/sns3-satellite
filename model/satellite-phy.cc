...
SatPhy::GetTypeId(void)
{
    static TypeId tid =
        TypeId("ns3::SatPhy")
            .SetParent<Object>()
            .AddAttribute("ReceiveCb",
                          "The receive callback for this phy.",
                          CallbackValue(),
                          MakeCallbackAccessor(&SatPhy::m_rxCallback),
                          MakeCallbackChecker())
            .AddAttribute("CnoCb",
                          "The C/N0 info callback for this phy.",
                          CallbackValue(),
                          MakeCallbackAccessor(&SatPhy::m_cnoCallback),
                          MakeCallbackChecker())
            .AddAttribute("AverageNormalizedOfferedLoadCallback",
                          "The average offered random access load callback for this phy.",
                          CallbackValue(),
...
SatPhy::Initialize()
{
    ...
    double eirpWoGainDbw =
        m_txMaxPowerDbw - m_txOutputLossDb - m_txPointingLossDb - m_txOboLossDb - m_txAntennaLossDb;
    ...
    {
        m_phyRx->SetAverageNormalizedOfferedLoadCallback(
            MakeCallback(&SatPhy::AverageNormalizedOfferedRandomAccessLoadInfo, this));
    }
...
void
SatPhy::SendPdu(PacketContainer_t p,
                uint32_t carrierId,
                Time duration,
...
SatPhy::ModcodTrace(Ptr<SatSignalParameters> rxParams)
{
    ...
    if (m_isStatisticsTagsEnabled)
    {
        ...
        case SatEnums::LD_RETURN: {
            SatSignalParameters::PacketsInBurst_t::iterator it1;
            for (it1 = rxParams->m_packetsInBurst.begin(); it1 != rxParams->m_packetsInBurst.end();
                 ++it1)
            {
...
void
SatPhy::AverageNormalizedOfferedRandomAccessLoadInfo(uint32_t satId,
                                                     uint32_t beamId,
                                                     uint32_t carrierId,
                                                     uint8_t allocationChannelId,
                                                     double averageNormalizedOfferedLoad)
{
    NS_LOG_FUNCTION(this << satId << beamId << carrierId << allocationChannelId
                         << averageNormalizedOfferedLoad);
    m_avgNormalizedOfferedLoadCallback(satId,
                                       beamId,
                                       carrierId,
                                       allocationChannelId,
                                       averageNormalizedOfferedLoad);
