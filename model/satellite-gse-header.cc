...
{
    static TypeId tid =
        TypeId("ns3::SatGseHeader").SetParent<Header>().AddConstructor<SatGseHeader>();
    return tid;
}

uint32_t
SatGseHeader::GetSerializedSize() const
{
...
void
SatGseHeader::Serialize(Buffer::Iterator start) const
{
    NS_LOG_FUNCTION(this);
    ...
    // First two bytes
    i.WriteU16((m_startIndicator << 15) | (m_endIndicator << 14) | (0x0 << 13) | (0x0 << 12) |
               (m_gsePduLengthInBytes & 0x0FFF));
...
uint32_t
SatGseHeader::Deserialize(Buffer::Iterator start)
{
    NS_LOG_FUNCTION(this);
...
void
SatGseHeader::Print(std::ostream& os) const
{
    NS_LOG_FUNCTION(this);

    os << m_startIndicator << " " << m_endIndicator << " " << m_gsePduLengthInBytes << " "
       << m_fragmentId << " " << m_totalLengthInBytes << " " << m_protocolType << " " << m_labelByte
       << " " << m_crc << std::endl;
}
...
uint8_t
SatGseHeader::GetStartIndicator() const
{
    NS_LOG_FUNCTION(this);
...
uint8_t
SatGseHeader::GetEndIndicator() const
{
    NS_LOG_FUNCTION(this);
...
uint32_t
SatGseHeader::GetGsePduLength() const
{
    NS_LOG_FUNCTION(this);
...
uint32_t
SatGseHeader::GetFragmentId() const
{
    NS_LOG_FUNCTION(this);
    return m_fragmentId;
}
...
uint32_t
SatGseHeader::GetTotalLength() const
{
    NS_LOG_FUNCTION(this);
...
void
SatGseHeader::SetStartIndicator()
{
    NS_LOG_FUNCTION(this);
    m_startIndicator = 1;
}
...
void
SatGseHeader::SetGsePduLength(uint32_t bytes)
{
...
void
SatGseHeader::SetFragmentId(uint32_t id)
{
...
void
SatGseHeader::SetTotalLength(uint32_t bytes)
{
    NS_LOG_FUNCTION(this << bytes);
...
uint32_t
SatGseHeader::GetGseHeaderSizeInBytes(uint8_t type) const
{
...
uint32_t
SatGseHeader::GetMaxGseHeaderSizeInBytes() const
{
    NS_LOG_FUNCTION(this);
