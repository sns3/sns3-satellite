...
SatQueue::GetQueueStatistics(bool reset)
{
    ...
    {
        queueStats.m_incomingRateKbps = SatConstVariables::BITS_PER_BYTE * m_nEnqueBytesSinceReset /
                                        (double)(SatConstVariables::BITS_IN_KBIT) /
                                        duration.GetSeconds();
        queueStats.m_outgoingRateKbps = SatConstVariables::BITS_PER_BYTE * m_nDequeBytesSinceReset /
                                        (double)(SatConstVariables::BITS_IN_KBIT) /
                                        duration.GetSeconds();
...
SatQueue::GetNumSmallerPackets(uint32_t maxPacketSizeBytes) const
{
    ...
    uint32_t packets(0);
    for (PacketContainer_t::const_iterator it = m_packets.begin(); it != m_packets.end(); ++it)
    {
