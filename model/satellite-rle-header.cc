...
{
    static TypeId tid =
        TypeId("ns3::SatPPduHeader").SetParent<Header>().AddConstructor<SatPPduHeader>();
    return tid;
}

uint32_t
SatPPduHeader::GetSerializedSize() const
{
...
void
SatPPduHeader::Serialize(Buffer::Iterator start) const
{
    NS_LOG_FUNCTION(this);
...
uint32_t
SatPPduHeader::Deserialize(Buffer::Iterator start)
{
    NS_LOG_FUNCTION(this);
...
void
SatPPduHeader::Print(std::ostream& os) const
{
    NS_LOG_FUNCTION(this);

    os << m_startIndicator << " " << m_endIndicator << " " << m_ppduLengthInBytes << " "
       << m_fragmentId << " " << m_totalLengthInBytes;
}
...
uint8_t
SatPPduHeader::GetStartIndicator() const
{
    NS_LOG_FUNCTION(this);
...
uint8_t
SatPPduHeader::GetEndIndicator() const
{
    NS_LOG_FUNCTION(this);
...
uint16_t
SatPPduHeader::GetPPduLength() const
{
    NS_LOG_FUNCTION(this);
...
uint8_t
SatPPduHeader::GetFragmentId() const
{
    NS_LOG_FUNCTION(this);
...
uint16_t
SatPPduHeader::GetTotalLength() const
{
    NS_LOG_FUNCTION(this);
...
void
SatPPduHeader::SetStartIndicator()
{
    NS_LOG_FUNCTION(this);
...
void
SatPPduHeader::SetPPduLength(uint16_t bytes)
{
...
void
SatPPduHeader::SetFragmentId(uint8_t id)
{
...
void
SatPPduHeader::SetTotalLength(uint16_t bytes)
{
    NS_LOG_FUNCTION(this << bytes);
...
uint32_t
SatPPduHeader::GetHeaderSizeInBytes(uint8_t type) const
{
...
uint32_t
SatPPduHeader::GetMaxHeaderSizeInBytes() const
{
    NS_LOG_FUNCTION(this);

    return std::max(std::max(m_startPpduHeaderSize, m_continuationPpduHeaderSize),
