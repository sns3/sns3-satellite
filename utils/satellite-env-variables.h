...
class SatEnvVariables : public Object
{
  public:
    ...
    /**
     * \brief Function for setting the output variables. The function also creates the output folder
     * based on the new variables. NOTICE: this function is meant to me used only in test cases,
     * where issues with singletons might arise. In any other case the class attributes should be
     * used instead. \param campaignName simulation campaign name \param simTag simulation tag
     * \param enableOutputOverwrite is simulation output overwrite enabled
     */
    void SetOutputVariables(std::string campaignName,
                            std::string simTag,
                            bool enableOutputOverwrite);

    /**
     * \brief Initialize class
     * NOTICE: this function is meant to me used only in test cases, where issues with singletons
     * might arise.
     */
    void DoInitialize();
  ...
  private:
     ...
     */
    std::string InitializeOutputFolders(std::string campaignName,
                                        std::string simTag,
                                        bool enableOutputOverwrite);

    /**
     * \brief Function for outputting the most essential simulation specific environmental
     * information
     */
     ...
     */
    void ExecuteCommandAndReadOutput(std::string command,
     ...
     * \return path to the created folder
     */
    std::string FormOutputPath(std::string simRootPath,
