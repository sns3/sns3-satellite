    ...
    m_timeColumn ()
{
  ...
  NS_FATAL_ERROR ("SatInputFileStreamTimeLongDoubleContainer - Constructor not in use");
}
...
void
SatInputFileStreamTimeLongDoubleContainer::UpdateContainer (std::string filename, std::ios::openmode filemode, uint32_t valuesInRow)
{
  NS_LOG_FUNCTION (this);
  ...
  if (m_container.size () < 1)
  ...
  else if (m_container.size () == 1)
    {
      if (m_container[m_container.size () - 1].at (m_timeColumn) == 0)
        {
          NS_FATAL_ERROR ("SatInputFileStreamTimeLongDoubleContainer::UpdateContainer - Invalid input file format (time sample error)");
        }
    }
  else
    {
      long double tempValue1 = m_container[0].at (m_timeColumn);
...
SatInputFileStreamTimeLongDoubleContainer::ProceedToNextClosestTimeSample ()
{
  ...
  if (m_numOfPasses > 0)
    {
      std::cout << "WARNING! - SatInputFileStreamTimeLongDoubleContainer for " << m_fileName << " is out of samples @ time sample " << Now ().GetSeconds () << " (passes "<< m_numOfPasses << ")" << std::endl;
...
bool
SatInputFileStreamTimeLongDoubleContainer::FindNextClosest (uint32_t lastValidPosition, uint32_t column, long double shiftValue, long double comparisonValue)
{
  NS_LOG_FUNCTION (this);
  ...
  for (uint32_t i = lastValidPosition; i < m_container.size (); i++)
    {
      if (m_container[i].at (column) + shiftValue >= comparisonValue)
        {
          long double difference1 = fabs (m_container[lastValidPosition].at (column) + shiftValue - comparisonValue);
          long double difference2 = fabs (m_container[i].at (column) + shiftValue - comparisonValue);
  ...
  if (valueFound && m_numOfPasses > 0 && m_currentPosition == 0)
    {
      long double difference1 = fabs (m_container[m_currentPosition].at (column) + shiftValue - comparisonValue);
      long double difference2 = fabs (m_container[m_container.size() - 1].at (column) + ((m_numOfPasses - 1) * m_container[m_container.size () - 1].at (column)) - comparisonValue);
