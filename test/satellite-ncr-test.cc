  ...
  for (uint32_t i = indexSwitchTdmaSync; i < m_states.size () - 1; i++)
    {
      ...
      NS_TEST_ASSERT_MSG_NE (m_states[i], SatUtMacState::READY_FOR_LOGON, "UT should not switch to READY_FOR_LOGON after NCR_RECOVERY");
    }
...
SatNcrTest3::DoRun (void)
{
  ...
  NS_TEST_ASSERT_MSG_NE (indexSwitchTdmaSync, 0, "UT should switch to TDMA_SYNC before the end of simulation");
  NS_TEST_ASSERT_MSG_NE (indexSwitchNcrRecovery, 0, "UT should switch to NCR_RECOVERY before the end of simulation");
  NS_TEST_ASSERT_MSG_NE (indexSwitchTdmaSyncSecondTime, 0, "UT should switch to TDMA_SYNC after NCR_RECOVERY and before the end of simulation");
  ...
  for (uint32_t i = indexSwitchNcrRecovery; i < m_states.size () - 1; i++)
    {
      if (m_states[i] == SatUtMacState::OFF_STANDBY || m_states[i] == SatUtMacState::READY_FOR_LOGON)
        {
          stateFound = true;
        }
    }
  NS_TEST_ASSERT_MSG_EQ (stateFound, true, "UT should switch to OFF_STANDBY or READY_FOR_LOGON after NCR_RECOVERY");
  ...
  for (uint32_t i = 31; i < 56; i++)
    {
      NS_TEST_ASSERT_MSG_EQ ((uint32_t) m_totalReceived[i], receivedBeforeNcrRecovery, "Receiver should not receive anything between 31s and 50s");
    }

  // Receiver has always received data after recovering NCR messages
  for (uint32_t i = indexSwitchTdmaSyncSecondTime + 2; i < m_totalReceived.size () - 1; i++)
    {
      NS_TEST_ASSERT_MSG_GT (m_totalReceived[i+1], m_totalReceived[i], "Receiver should always receive data after second logon");
    }
...
SatNcrTest3::ChangeTxStatus (bool enable)
{
  if (enable)
    ...
    {
      DynamicCast<SatGwMac> (DynamicCast<SatNetDevice> (m_helper->GwNodes ().Get (0)->GetDevice (1))->GetMac ())->SetAttribute ("NcrBroadcastPeriod", TimeValue (Seconds (30)));
    }
}
