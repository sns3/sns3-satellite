...
SatRtnArqTestCase::Initialize()
{
    ...
    m_rle =
        CreateObject<SatReturnLinkEncapsulatorArq>(m_source, m_dest, m_source, m_dest, m_rcIndex);
...
SatRtnArqTestCase::DoRun(void)
{
    ...
    NS_TEST_ASSERT_MSG_LT(errorProb,
                          m_errorProbabilityThreshold,
                          "HL packet error probability is higher than threshold!");

    // std::cout << "RTN: Sent packets: " << m_sentPacketSizes.size () << " received packets: " <<
    // m_rcvdPacketSizes.size () << std::endl; std::cout << "Tx: " << m_txs << " txErrors: " <<
    // m_txErrors << " Acks: " << m_acks << " ackErrors: " << m_ackErrors << std::endl;
...
void
SatRtnArqTestCase::TransmitPdus(uint32_t numPackets)
{
...
void
SatRtnArqTestCase::NotifyTxOpportunity()
{
    /**
    ...
    {
        // Schedule next TxO
        Time nextTxOTime = m_txoInterval + MilliSeconds(m_unif->GetValue(m_minTimeSlotJitterInMs,
...
void
SatRtnArqTestCase::ReceivePdu(Ptr<Packet> p)
{
    m_rle->ReceivePdu(p);
}

bool
SatRtnArqTestCase::SendAck(Ptr<SatControlMessage> msg, const Address& dest)
{
    m_acks++;
...
void
SatRtnArqTestCase::ReceiveAck(Ptr<SatArqAckMessage> ack)
{
    m_rle->ReceiveAck(ack);
}

void
SatRtnArqTestCase::Receive(Ptr<Packet> p, Mac48Address source, Mac48Address dest)
{
    uint32_t rcvdPacketSize = p->GetSize();
    m_rcvdPacketSizes.push_back(rcvdPacketSize);

    // std::cout << "Now: " << Now ().GetSeconds () << " sent: " << m_sentPacketSizes.at
    // (m_rcvdPacketSizes.size ()-1) << " Rcvd: " << rcvdPacketSize << std::endl;
}
...
SatFwdArqTestCase::Initialize()
{
    ...
    Config::SetDefault("ns3::SatGenericStreamEncapsulatorArq::RxWaitingTime",
                       TimeValue(Seconds(2.3)));

    Ptr<SatQueue> queue = CreateObject<SatQueue>(m_flowIndex);
    m_gse = CreateObject<SatGenericStreamEncapsulatorArq>(m_source,
                                                          m_dest,
                                                          m_source,
                                                          m_dest,
                                                          m_flowIndex);
...
SatFwdArqTestCase::DoRun(void)
{
    ...
    // std::cout << "FWD: Sent packets: " << m_sentPacketSizes.size () << " received packets: " <<
    // m_rcvdPacketSizes.size () << std::endl; std::cout << "Tx: " << m_txs << " txErrors: " <<
    // m_txErrors << " Acks: " << m_acks << " ackErrors: " << m_ackErrors << std::endl;
...
void
SatFwdArqTestCase::TransmitPdus(uint32_t numPackets)
{
    // Create packets and push them to RLE
    for (uint32_t i = 0; i < numPackets; ++i)
    {
...
void
SatFwdArqTestCase::NotifyTxOpportunity()
{
    /**
    ...
    if (!bbFrame.empty())
    {
        ...
        {
            for (std::vector<Ptr<Packet>>::iterator it = bbFrame.begin(); it != bbFrame.end(); ++it)
            {
                // Schedule packet receive
...
void
SatFwdArqTestCase::ReceivePdu(Ptr<Packet> p)
{
    m_gse->ReceivePdu(p);
}

bool
SatFwdArqTestCase::SendAck(Ptr<SatControlMessage> msg, const Address& dest)
{
    m_acks++;
...
void
SatFwdArqTestCase::ReceiveAck(Ptr<SatArqAckMessage> ack)
{
    m_gse->ReceiveAck(ack);
}

void
SatFwdArqTestCase::Receive(Ptr<Packet> p, Mac48Address source, Mac48Address dest)
{
    uint32_t rcvdPacketSize = p->GetSize();
    m_rcvdPacketSizes.push_back(rcvdPacketSize);
