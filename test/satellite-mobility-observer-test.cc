 ...
 *
 *  Notes:
 *    At least corner points, valid values and out of range (NAN) elevation angle values is needed
 * to test. When reference satellite is used for example MatLab is useful to get reference values
...
SatMobilityObserverTestCase::DoRun(void)
{
    ...
    Ptr<SatConstantPropagationDelayModel> gwProgDelay =
        CreateObject<SatConstantPropagationDelayModel>();
    Ptr<SatConstantPropagationDelayModel> utProgDelay =
        CreateObject<SatConstantPropagationDelayModel>();
    ...
    for (uint32_t i = 0; i < g_latitudeCount; i++)
    {
        for (uint32_t j = 0; j < g_longitudeCount; j++)
        {
            ...
            if (refValue < 0)
            {
                // NAN should be returned, because only points where we can see satellite is
                // accepted
                NS_TEST_ASSERT_MSG_EQ(std::isnan(gwEl),
                                      true,
                                      "UT elevation angle incorrect: lat-> " << lat << " lon-> "
                                                                             << " alt->" << alt);
            }
            else
            {
