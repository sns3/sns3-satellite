                  ...
                  double cSinr)
{
    ...
    if (!destAdd.IsBroadcast())
    {
        ...
        std::cout << "Time= " << Simulator::Now() << ", ChType= " << std::setw(17)
                  << SatEnums::GetChannelTypeName(params->m_channelType) << ", OwnAddr= " << ownAdd
                  << ", DestAddr= " << destAdd << ", Beam= " << std::setw(2) << params->m_beamId
                  << ", Freq= " << params->m_carrierFreq_hz << ", IFPwr= " << std::setw(8)
                  << SatUtils::WToDbW<double>(ifPower) << ", RXPwr= " << std::setw(8)
                  << SatUtils::WToDbW<double>(params->m_rxPower_W) << ", SINR= " << std::setw(7)
                  << SatUtils::LinearToDb<double>(params->GetSinr()) << ", CSINR= " << std::setw(7)
                  << SatUtils::LinearToDb<double>(cSinr) << std::endl;
    }
}
...
class SatPerPacketBaseTestCase : public TestCase
{
  public:
    ...
    SatPerPacketBaseTestCase(std::string name,
                             std::string extName,
                             SatEnums::FadingModel_t fading,
                             bool dummyFrames,
                             StringValue ival1,
                             UintegerValue pSize1,
                             StringValue ival2,
                             UintegerValue pSize2);
...
SatPerPacketBaseTestCase::SatPerPacketBaseTestCase(std::string name,
                                                   std::string extName,
                                                   SatEnums::FadingModel_t fading,
                                                   bool dummyFrames,
                                                   StringValue ival1,
...
SatPerPacketBaseTestCase::InitOutput(bool figureOutput)
{
    ...
    Ptr<SatInterferenceOutputTraceContainer> ptrCont =
        Singleton<SatInterferenceOutputTraceContainer>::Get();
    ptrCont->Reset();
 ...
 *    2.  User Node-1 sends packets continuously, transmitting constant bitrate (CBR) to user
 * Node-3.
 *    3.  User Node-2 sends short packets with e.g. 100ms delay between packets to user Node-4.
 *
 *  Expected result:
 *    UT serving Node-3 should see interference only in some of the received packet (when Node-2 is
 * sending). UT serving Node-4 should see inference in every packet it receives.
 *
 *  Notes:
 *    Node-1 and Node-2 are attached to different IP router (different GW nodes) and Node-3 and
 * Node-4 are attached to different UT nodes. Node-1 and Node-2 must be selected from different
 * beams, so that packet sending happens in same channel at least in feeder or user link.
 */

// Default name to show for SatPerPacketFwdLinkUserTestCase test. When the second constructor is
// used, the text to append this name can be given.
...
class SatPerPacketFwdLinkUserTestCase : public SatPerPacketBaseTestCase
{
  public:
    ...
    SatPerPacketFwdLinkUserTestCase(std::string name,
                                    SatEnums::FadingModel_t fading,
                                    bool dummyFrames);
    SatPerPacketFwdLinkUserTestCase(std::string name,
                                    SatEnums::FadingModel_t fading,
...
SatPerPacketFwdLinkUserTestCase::SatPerPacketFwdLinkUserTestCase()
    : SatPerPacketBaseTestCase(defFwdUserName,
                               "IfTestFwdUsr",
                               SatEnums::FADING_OFF,
                               false,
                               StringValue("0.00015s"),
                               UintegerValue(512),
                               StringValue("0.1s"),
                               UintegerValue(512))
{
...
SatPerPacketFwdLinkUserTestCase::SatPerPacketFwdLinkUserTestCase(std::string name,
                                                                 SatEnums::FadingModel_t fading,
                                                                 bool dummyFrames)
    : SatPerPacketBaseTestCase(defFwdUserName + name,
                               "IfTestFwdUsr_" + name,
                               fading,
                               dummyFrames,
                               StringValue("0.00015s"),
...
SatPerPacketFwdLinkUserTestCase::DoRun(void)
{
    ...
    sinkHelper.SetAttribute(
        "Local",
        AddressValue(Address(InetSocketAddress(helper->GetUserAddress(utUsers.Get(1)), port))));
...
// Default name to show for SatPerPacketFwdLinkFullTestCase test. When the second constructor is
// used, the text to append this name can be given.
...
class SatPerPacketFwdLinkFullTestCase : public SatPerPacketBaseTestCase
{
  public:
    ...
    SatPerPacketFwdLinkFullTestCase(std::string name,
                                    SatEnums::FadingModel_t fading,
                                    bool dummyFrames);
    SatPerPacketFwdLinkFullTestCase(std::string name,
                                    SatEnums::FadingModel_t fading,
...
SatPerPacketFwdLinkFullTestCase::SatPerPacketFwdLinkFullTestCase()
    : SatPerPacketBaseTestCase(defFwdFullName,
                               "IfTestFwdFull",
                               SatEnums::FADING_OFF,
                               false,
                               StringValue("0.00015s"),
...
SatPerPacketFwdLinkFullTestCase::SatPerPacketFwdLinkFullTestCase(std::string name,
                                                                 SatEnums::FadingModel_t fading,
                                                                 bool dummyFrames)
    : SatPerPacketBaseTestCase(defFwdFullName + name,
                               "IfTestFwdFull_" + name,
                               fading,
                               dummyFrames,
                               StringValue("0.00015s"),
 ...
 *    2.  User Node-3 sends packets continuously, transmitting constant bitrate (CBR) to user
 * Node-1.
 *    3.  User Node-4 sends short packets with e.g. 100ms delay between packets to user Node-2.
 *
 *  Expected result:
 *    GW serving Node-1 should see interference only in some of the received packet (when Node-4 is
 * sending). GW serving Node-2 should see inference in every packet it receives.
 *
 *  Notes:
 *    Node-1 and Node-2 are attached to IP router (different GW nodes) and Node-3 and Node-4 are
 * attached to different UT nodes. Node-1 and Node-2 must be selected from different beams, so that
 * packet sending happens in same channel at least in user or feeder link.
 */

// Default name to show for SatPerPacketRtnLinkUserTestCase test. When the second constructor is
// used, the text to append this name can be given.
...
class SatPerPacketRtnLinkUserTestCase : public SatPerPacketBaseTestCase
{
  public:
    ...
    SatPerPacketRtnLinkUserTestCase(std::string name,
                                    SatEnums::FadingModel_t fading,
                                    StringValue ival1,
                                    UintegerValue pSize1,
                                    StringValue ival2,
...
SatPerPacketRtnLinkUserTestCase::SatPerPacketRtnLinkUserTestCase()
    : SatPerPacketBaseTestCase(defRtnUserName,
                               "IfTestRtnUsr",
                               SatEnums::FADING_OFF,
                               false,
                               StringValue("0.01s"),
...
SatPerPacketRtnLinkUserTestCase::SatPerPacketRtnLinkUserTestCase(std::string name,
                                                                 SatEnums::FadingModel_t fading)
    : SatPerPacketBaseTestCase(defRtnUserName + name,
                               "IfTestRtnUsr_" + name,
                               fading,
                               false,
                               StringValue("0.01s"),
...
class SatPerPacketRtnLinkFullTestCase : public SatPerPacketBaseTestCase
{
  public:
    ...
    SatPerPacketRtnLinkFullTestCase(std::string name,
                                    SatEnums::FadingModel_t fading,
                                    StringValue ival1,
                                    UintegerValue pSize1,
                                    StringValue ival2,
...
SatPerPacketRtnLinkFullTestCase::SatPerPacketRtnLinkFullTestCase()
    : SatPerPacketBaseTestCase(defRtnFullName,
                               "IfTestRtnFull",
                               SatEnums::FADING_OFF,
                               false,
                               StringValue("0.01s"),
...
SatPerPacketRtnLinkFullTestCase::SatPerPacketRtnLinkFullTestCase(std::string name,
                                                                 SatEnums::FadingModel_t fading)
    : SatPerPacketBaseTestCase(defRtnFullName + name,
                               "IfTestRtnFull_" + name,
                               fading,
                               false,
                               StringValue("0.01s"),
