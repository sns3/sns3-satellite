 ...
 *
 *  This case tests that SatConstantPositionMobilityModel object can be created successfully and
 * position allocation can be done with random box allocator.
 *    1.  Create SatConstantPositionMobilityModel object with SatRandomBoxPositionAllocator with
 * MobilityHelper.
 ...
 *
 *  This case tests that SatConstantPositionMobilityModel object can be created successfully and
 * position allocation can be done with list allocator when they operate using geodetic coordinates.
 *    1.  Create SatListPositionAllocator object and set positions to it.
 *    2.  Create SatConstantPositionMobilityModel object with SatListPositionAllocator with
 * MobilityHelper.
 ...
 *
 *  Notes:
 *    SatMobilityModel and SatListPositionAllocator (SatPositionAllocator) attributes
 * AsGeoCoordinates define, if mobility and allocator operate using geodetic coordinates or using
 * Cartesian Coordinates. By default geodetic coordinates is used.
 *
 */
 ...
 *  This case tests that SatConstantPositionMobilityModel object can be created successfully and
 * position allocation can be done with list allocator when they operate using geodetic coordinates.
 *    1.  Configure SatConstantPositionMobilityModel and SatListPositionAllocator to use Cartesian
 * coordinates.
 *    2.  Create SatListPositionAllocator object and set positions to it.
 *    3.  Create SatConstantPositionMobilityModel object with SatListPositionAllocator with
 * MobilityHelper.
 ...
 *  Expected result:
 *    Positions should be same as set to allocator.
 *    But due to needed conversion they are not necessarily exactly same, so some tolerance is
 * accepted.
 *
 *  Notes:
 *    SatMobilityModel and SatListPositionAllocator (SatPositionAllocator) attributes
