 ...
 * 3.  After simulation trace results, memory consumption reports and execution time saved for
 * further analysing.
 *
 ...
 * for tracking and analysing purposes. Valgrind program is utilized in test to validate memory
 * usage. Test result is referred to previous results and analysed that execution time and memory
 * consumption doesn’t increase unacceptable level. The case helps recognizing of possible
...
Pm1::Pm1()
    : TestCase("'Performance and memory tracking' tests servers as follow-up test for performance "
               "and memory usage.")
{
}
...
Pm1::DoRun(void)
{
    ...
    NS_TEST_ASSERT_MSG_EQ(gwReceiver->GetTotalRx(),
                          utSender->GetSent(),
                          "Packets were lost between UT and GW!");
